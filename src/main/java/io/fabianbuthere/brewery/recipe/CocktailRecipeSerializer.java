...
public class CocktailRecipeSerializer implements RecipeSerializer<CocktailRecipe> {
    ...
    @Override
    public CocktailRecipe fromJson(ResourceLocation pRecipeId, JsonObject pSerializedRecipe) {
        JsonObject data = pSerializedRecipe.getAsJsonObject("cocktail_data");

        List<CocktailRecipe.BrewInput> brewInputs = new ArrayList<>();
        ...
        for (JsonElement el : brewArray) {
            ...
            brewInputs.add(new CocktailRecipe.BrewInput(brewTypeId, count));
        }
        ...
        return new CocktailRecipe(pRecipeId, brewInputs, extras, resultBrewTypeId);
    }

    @Override
    public @Nullable CocktailRecipe fromNetwork(ResourceLocation pRecipeId, FriendlyByteBuf pBuffer) {
    ...
    @Override
    public void toNetwork(FriendlyByteBuf pBuffer, CocktailRecipe pRecipe) {
        pBuffer.writeVarInt(pRecipe.getBrewInputs().size());
        for (CocktailRecipe.BrewInput input : pRecipe.getBrewInputs()) {
